...
-- ==============================================================================
create or replace package body sert_core.data_api
as
...
-- ==============================================================================
procedure push (p_text in varchar2 default null)
is
begin
...
-- ==============================================================================
procedure import_categories (p_rules in blob)
is
    ...
    for c in (
        select category_key,
         ...
         where not exists (
             select null
               from categories c
              where c.category_key = x.category_key
         )
         order by 1
    )
    loop
...
-- ==============================================================================
procedure import_rule_severity (p_rules in blob)
is
    l_count pls_integer := 0;
begin
    for c in (
        select rule_severity_key,
               rule_severity_name
          from (
              select distinct
                     rule_severity_key,
         ...
         where not exists (
             select null
               from rule_severity rs
              where rs.rule_severity_key = x.rule_severity_key
         )
         order by 1
    )
    loop
        ...
        l_count := l_count + 1;
        insert into rule_severity (
            rule_severity_name,
            rule_severity_key
        )
        values (
            c.rule_severity_name,
            c.rule_severity_key
        );
...
is
    ...
    procedure load_rule_severity_id
    is
    begin
        select rule_severity_id
          into l_rule_severity_id
          from rule_severity
         where rule_severity_key = p_rule_severity_key;
    exception
        when no_data_found then
            insert into rule_severity (
                rule_severity_name,
    ...
    procedure load_rule_criteria_type_id
    is
    begin
    ...
    procedure load_builder_url_id
    is
    begin
        select builder_url_id
          into l_builder_url_id
          from builder_urls
         where builder_url_key = p_builder_url_key;
    exception
        when no_data_found then
            null;
    end load_builder_url_id;

    procedure load_risk_id
    is
    begin
...
begin
    ...
    select json_doc
      into l_clob
      from rules_relational_to_json_v;
    ...
    for c in (
        select r.rule_id,
          ...
          from json_table(
               ...
               join categories c on j.category_key = c.category_key
               join rule_severity rs on j.rule_severity_key = rs.rule_severity_key
               left join rules r on j.rule_key = r.rule_key
               left join rule_criteria_types rct on j.rule_criteria_type_key = rct.rule_criteria_type_key
               left join builder_urls u on j.builder_url_key = u.builder_url_key
               left join risks rsk on j.risk_code = rsk.risk_code
    )
    loop
    ...
    if l_rule_set_id is not null then
        ...
        for i in (
            select rule_id
             ...
             where apex_version = p_apex_version
        )
        loop
            ...
            if l_found = 'N' then
                ...
                insert into sert_core.rule_set_rules (
                    rule_set_id,
                    rule_id
                )
                values (
