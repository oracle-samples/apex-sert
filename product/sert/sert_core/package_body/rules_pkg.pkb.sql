...
as
  ...
  -- purpose: import rules from a JSON upload into relational tables, creating lookup rows when missing.
  -- behavior: sets a log key; creates APEX collection 'RULES'; iterates rules_json_to_relational_v by p_name; ensures
  --   category, severity, and builder url exist (creating when necessary); looks up risk by code; inserts new rules or
  --   updates existing ones by (rule_key, apex_version); logs outcomes and adds collection members; errors are logged and re-raised.
  -- parameters:
  --   p_name - uploaded file identifier consumed by rules_json_to_relational_v
  ----------------------------------------------------------------------------------------------------------------------------
  procedure import
    (
    p_name in varchar2
  ...
  -- purpose: export all rules to JSON for download.
  -- behavior: sets log key; logs start/end; selects JSON CLOB from rules_relational_to_json_v, converts to BLOB, streams
  --   via OWA with appropriate headers; stops APEX engine after sending response.
  -- parameters: none
  ----------------------------------------------------------------------------------------------------------------------------
  procedure export
  is
  ...
  begin
    select replace(apex_string_util.get_slug('APEX-SERT_Rules_' || sert_apex_version), '-', '_') || '.json' filename
      into l_filename
      from apex_version_v;
  ...
  -- purpose: assign a rule to one or more rule sets.
  -- behavior: splits p_rule_sets on ':' into an array and inserts rows into rule_set_rules for each id.
  -- parameters:
  --   p_rule_id    - target rule id
  --   p_rule_sets  - colon-delimited list of rule_set_id values
  ----------------------------------------------------------------------------------------------------------------------------
  ...
  -- purpose: duplicate an existing rule under a new name/key and optionally assign to rule sets.
  -- behavior: selects the source rule by p_rule_id and inserts a copy using p_rule_name/p_rule_key; returns new rule_id;
  --   when p_rule_sets provided, assigns the new rule to each listed rule_set_id.
  -- parameters:
  --   p_rule_id    - in out; updated to new rule id
  --   p_rule_name  - name for the new rule
  --   p_rule_key   - key for the new rule
  --   p_rule_sets  - optional colon-delimited list of rule_set_id values
  ----------------------------------------------------------------------------------------------------------------------------
  procedure copy_rule
    (
  ...
  ----------------------------------------------------------------------------------------------------------------------------
  -- upgrade_rule
  -- purpose: copy a rule to a specified APEX version when an equivalent does not already exist and the source is active and not internal.
  -- behavior: for the given p_rule_id, selects the source rule; when not already present in p_apex_version, inserts a versioned
  --   copy and returns the new rule_id; if p_rule_sets provided, assigns the new rule to each listed rule_set_id.
  -- parameters:
  --   p_rule_id      - in out; updated with new rule id
  --   p_apex_version - target APEX version (varchar2)
  --   p_rule_sets    - optional colon-delimited list of rule_set_id values to assign
  ----------------------------------------------------------------------------------------------------------------------------
  ...
  -- purpose: bulk-upgrade rules and rule sets to the current APEX version, preserving assignments and exceptions.
  -- behavior:
  --   - Determine current target APEX version (nv('G_APEX_VERSION') or apex_version_v) and previous version from prefs.
  --   - If no rules exist for target version, seed by copying all rules from previous version where missing.
  --   - For each active rule set in previous version without a target version counterpart, create the new rule set; map
  --     rule_set_rules to equivalent rules at target version (creating copies for non-internal rules as needed via upgrade_rule);
  --     deactivate source rules and the previous rule set; copy exceptions to the new rule set.
  --   - Update prefs.SERT_APEX_VERSION to the target version.
  -- parameters: none
  ----------------------------------------------------------------------------------------------------------------------------
