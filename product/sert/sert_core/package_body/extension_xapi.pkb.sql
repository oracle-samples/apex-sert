...
begin
   ...
   for rec in ( select workspace
                 ...
                 where aw.workspace <> upper(p_to_workspace)
                   and not exists ( select workspace_name
                                      from apex_workspace_schemas ws
                                     where ws.workspace_name = aw.workspace
                                       and schema = apex_application.g_flow_schema_owner )
                 minus
