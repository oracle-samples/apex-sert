set serveroutput on
set feedback off
spool shared_comp_views_merge.sql
declare
-->>>>>>>>>>>>>>>>>>>> CHANGE THE LINES BELOW <<<<<<<<<<<<<<<<<<<<<<<
   -- Table to merge in to (UPPER CASE)
   table_name    varchar2(30) := 'sert_core.shared_comp_views';
   ...
   column_list   varchar2(4000) := 'shared_comp_view, shared_comp_type';
   -- Columns used for the MERGE MATHCH (comma separated list)
   match_columns varchar2(4000) := 'shared_comp_view';
--^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
   cur           sys_refcursor;
   curid         number;
