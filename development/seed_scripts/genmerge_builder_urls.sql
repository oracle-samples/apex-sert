...
-->>>>>>>>>>>>>>>>>>>> CHANGE THE LINES BELOW <<<<<<<<<<<<<<<<<<<<<<<
   ...
   table_name    varchar2(30) := 'sert_core.builder_urls';
   ...
   column_list   varchar2(4000) := 'component_name, builder_url_key, data_type_id, data_link';
   -- Columns used for the MERGE MATHCH (comma separated list)
   match_columns varchar2(4000) := 'builder_url_key';
--^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
   cur           sys_refcursor;
   curid         number;
