set serveroutput on
set feedback off
spool rule_set_types_merge.sql
declare
-->>>>>>>>>>>>>>>>>>>> CHANGE THE LINES BELOW <<<<<<<<<<<<<<<<<<<<<<<
   -- Table to merge in to (UPPER CASE)
   table_name    varchar2(30) := 'sert_core.rule_set_types';
   ...
   column_list   varchar2(4000) := 'rule_set_type_name, rule_set_type_key';
   -- Columns used for the MERGE MATHCH (comma separated list)
   match_columns varchar2(4000) := 'rule_set_type_key';
--^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
   cur           sys_refcursor;
   curid         number;
