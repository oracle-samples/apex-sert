set serveroutput on
set feedback off
spool categories_merge.sql

declare
-->>>>>>>>>>>>>>>>>>>> CHANGE THE LINES BELOW <<<<<<<<<<<<<<<<<<<<<<<
   -- Table to merge in to (UPPER CASE)
   table_name    varchar2(30) := 'sert_core.categories';
   ...
   column_list   varchar2(4000) := 'category_name, category_key';
   -- Columns used for the MERGE MATHCH (comma separated list)
   match_columns varchar2(4000) := 'category_key';
--^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
   cur           sys_refcursor;
   curid         number;
