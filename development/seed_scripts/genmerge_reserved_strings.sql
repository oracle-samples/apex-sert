set serveroutput on
set feedback off
spool reserved_strings_merge.sql

declare
-->>>>>>>>>>>>>>>>>>>> CHANGE THE LINES BELOW <<<<<<<<<<<<<<<<<<<<<<<
   -- Table to merge in to (UPPER CASE)
   table_name    varchar2(30) := 'sert_core.reserved_strings';
   ...
   column_list   varchar2(4000) := 'reserved_string, reserved_string_key, reserved_string_type';
   -- Columns used for the MERGE MATHCH (comma separated list)
   match_columns varchar2(4000) := 'reserved_string_key';
--^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
   cur           sys_refcursor;
   curid         number;
