set serveroutput on
set feedback off
spool rule_severity_merge.sql
declare
-->>>>>>>>>>>>>>>>>>>> CHANGE THE LINES BELOW <<<<<<<<<<<<<<<<<<<<<<<
   -- Table to merge in to (UPPER CASE)
   table_name    varchar2(30) := 'sert_core.rule_severity';
   ...
   column_list   varchar2(4000) := 'rule_severity_name, rule_severity_key, seq';
   -- Columns used for the MERGE MATHCH (comma separated list)
   match_columns varchar2(4000) := 'rule_severity_key';
--^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
   cur           sys_refcursor;
   curid         number;
