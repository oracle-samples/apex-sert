set serveroutput on
set feedback off
spool prefs_merge.sql

declare
-->>>>>>>>>>>>>>>>>>>> CHANGE THE LINES BELOW <<<<<<<<<<<<<<<<<<<<<<<
   -- Table to merge in to (UPPER CASE)
   table_name    varchar2(30) := 'sert_core.prefs';
   ...
   column_list   varchar2(4000) := 'pref_name, pref_key, pref_value';
   -- Columns used for the MERGE MATHCH (comma separated list)
   match_columns varchar2(4000) := 'pref_key';
--^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
   cur           sys_refcursor;
   curid         number;
